...
# We get a named logger so it integrates with the root logger configured in cli.py.
gyre_logger = logging.getLogger('GYRE_Pipeline')
# The level for this logger will be inherited from the root logger,
# unless explicitly set here (e.g., gyre_logger.setLevel(logging.DEBUG))
...
def run_gyre_workflow(
    ...
    filtered_profiles_csv_path: str = None, # Optional, used only for FILTERED_PROFILES mode
    global_mesa_base_dir: str = '.', # Top-level MESA grid dir, read from cli.py's main config (input_dir)
    global_output_base_dir: str = './output', # Top-level output dir for the current session, read from cli.py
    ...
    Args:
        gyre_config_path (str): Path to the GYRE specific configuration file (e.g., 'gyre_config.in').
        filtered_profiles_csv_path (str, optional): Absolute path to the CSV file generated by mesa_analyzer,
                                                    containing profiles to run in 'FILTERED_PROFILES' mode.
                                                    Defaults to None.
    ...
    except FileNotFoundError: # Should ideally be caught by the initial check, but for robustness
    ...
    # Accessing settings from the gyre_config.in file
    setup_cfg = config.get('setup', {})
    gyre_cfg = config.get('gyre_options', {})

    # Validate essential configuration parameters from gyre_config.in
    # 'gyre_dir' is expected in the '[setup]' section
    ...
    # Define a dedicated GYRE output subdirectory within the main global_output_base_dir
    # This keeps GYRE's outputs organized under the current session's output folder
    ...
    if not (os.path.exists(gyre_executable) and os.path.isfile(gyre_executable) and os.access(gyre_executable, os.X_OK)):
        gyre_executable_fallback = os.path.join(gyre_install_dir, 'gyre') # Common alternative location
    ...
    # The template path can be relative to the gyre_config_path or absolute.
    # We resolve it relative to the directory containing gyre_config_path if it's not absolute.
    ...
    tasks = [] # List to hold (profile_path, inlist_template, output_dir, executable, threads) tuples
    
    run_mode = gyre_cfg['run_mode'].upper()

    if run_mode == 'ALL_PROFILES':
        # global_mesa_base_dir is the top-level directory containing all MESA runs (e.g., 'input_dir' from cli.py)
        # mesa_profile_base_dir is relative to global_mesa_base_dir (e.g., 'MESA_runs')
        ...
        if not all_profile_paths:
            ...
            return # Exit the function if no profiles are found
        else:
            for profile_path in all_profile_paths:
                # Create a specific output directory for each profile within the GYRE session output
        ...
        for index, row in filter_df.iterrows():
            # Safely convert mass and Z, handling potential NaN if not dropped above
            ...
            # Collect ALL profiles within the specified model number range
            ...
            # This ensures GYRE outputs for different MESA runs are separated
            mesa_run_basename = os.path.basename(mesa_run_specific_dir)
            specific_model_output_root = os.path.join(
                gyre_session_output_dir, # Use the GYRE specific output base
                mesa_run_basename
            )
            ...
            for prof_num in sorted(list(selected_profile_numbers_for_this_run)):
                ...
                if os.path.exists(profile_path):
                    # Create a unique output directory for each profile within its MESA run's GYRE output
...
if __name__ == "__main__":
    ...
    parser.add_argument('--global_mesa_base_dir', type=str, default=os.getcwd(), # Default to current dir for testing
                        help="Top-level MESA grid directory (e.g., input_dir from main config).")
    parser.add_argument('--global_output_base_dir', type=str, default='./standalone_gyre_output', # Default output for testing
    ...
    # Configure a basic logger for standalone execution if no handlers are already set
    if not gyre_logger.handlers:
        ...
        # Optional: Add a file handler if you want a log file for standalone runs
        # log_file_path = os.path.join(args.global_output_base_dir, 'standalone_gyre_modules.log')
        # os.makedirs(os.path.dirname(log_file_path), exist_ok=True)
        # gyre_logger.addHandler(logging.FileHandler(log_file_path))
