            ...
            try:
                ...
                for mode_idx in range(len(gyre_table)):
                    ...
                    new_row = {
                        ...
                        'n_p': mode_row_gyre['n_p'] if 'n_p' in mode_row_gyre.dtype.names else np.nan, # Added n_p
                        'n_g': mode_row_gyre['n_g'] if 'n_g' in mode_row_gyre.dtype.names else np.nan, # Added n_g
...
def plot_gyre_hrd(df_modes_filtered):
    ...
    Plots colored by 'n_p' (mode order, filtered to n_p <= 20) and 'eta'.
    """
    if df_modes_filtered.empty:
        print("No data available for plotting after filtering or collection.")
        return

    # --- Plot 1: HRD colored by n_p (filtered) ---
    plt.figure(figsize=(10, 8))

    # Filter for n_p <= 20 (and not NaN)
    df_np_filtered = df_modes_filtered[(df_modes_filtered['n_p'].notna()) & (df_modes_filtered['n_p'] <= 20)].copy()

    if not df_np_filtered.empty:
        # Convert n_p to integer and then string for categorical hue mapping
        # Only include unique integer values in the legend
        unique_np_values = sorted(df_np_filtered['n_p'].dropna().astype(int).unique())
        
        sns.scatterplot(data=df_np_filtered,
                        x='log_Teff',
                        y='log_L',
                        hue=df_np_filtered['n_p'].astype(str), # Convert to string for discrete colors
                        palette='viridis', # Colormap for n_p
                        s=50,
                        alpha=0.7,
                        edgecolor='w',
                        linewidth=0.5)
        ...
        plt.title(f'Hertzsprung-Russell Diagram - GYRE Modes by $n_p$ (up to $n_p=20$) ({len(df_np_filtered)} Modes)')
        plt.gca().invert_xaxis() # Invert x-axis for traditional HRD (Teff increases to the left)
        plt.grid(True, linestyle='--', alpha=0.6)
        
        # Manually create legend if unique_np_values are not too many
        if len(unique_np_values) < 30: # Limit legend entries for readability
            plt.legend(title=r'$n_p$ Mode Number', bbox_to_anchor=(1.05, 1), loc='upper left',
                       labels=[str(int(val)) for val in unique_np_values])
        else:
            plt.legend(title=r'$n_p$ Mode Number', bbox_to_anchor=(1.05, 1), loc='upper left')

        plt.tight_layout()
        plot_filename_np = os.path.join(plot_output_dir, "hrd_gyre_modes_by_np_filtered.png")
        plt.savefig(plot_filename_np, dpi=300)
        print(f"HRD plot (by $n_p$, filtered to $n_p \le 20$) saved to: {plot_filename_np}")
    else:
        print("No modes found for plotting HRD by n_p (after n_p <= 20 filter).")
...
if __name__ == '__main__':
    ...
    if not df_gyre_modes.empty:
        # You can filter data here if you only want to plot specific modes
        # Example: only l=0 modes:
        # df_modes_to_plot = df_gyre_modes[df_gyre_modes['l'] == 0]
        # Or all found modes:
