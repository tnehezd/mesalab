...
Plotting execution and layout are controlled by two distinct sections within your :ref:`YAML configuration <understanding_yaml_config>` file:
...
The Plotting workflow in `mesalab` is integrated with the data processing performed by the :doc:`mesa_analysis_workflow` It processes the generated CSVs (``summary_results.csv``, ``crossing_count_grid.csv``).
