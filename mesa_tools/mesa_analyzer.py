...
def perform_mesa_analysis(args, analysis_results_sub_dir, detail_files_output_dir):
    ...
    # Determine if reanalysis is needed based on force_reanalysis flag or missing files
    ...
    if not reanalysis_needed:
        ...
        try:
            ...
            # If analyze_blue_loop is True, try to load existing detail files for plotting
            # The load_and_group_data function would combine all detail files into one DF
                    ...
                    if analyze_blue_loop:
                        ...
                        if not pd.isna(analyzer_output['crossing_count']):
                            ...
                            if not analyzer_output['blue_loop_detail_df'].empty:
                                ...
                                # 'initial_mass' and 'initial_Z' are already added to df_full_history,
                                # and should be carried over to bl_df if it's a subset.
                                # Ensure they are present for summary stats if output_type is 'all'
                                if blue_loop_output_type == 'all':
                                    ...
                                    elif 'log_R' in df_full_history.columns: # Fallback in case bl_df filtered it out
